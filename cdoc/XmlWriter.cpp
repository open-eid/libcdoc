...
int64_t XMLWriter::writeStartElement(NS ns, const std::string &name, const std::map<std::string, std::string> &attr)
{
    ...
    auto pos = nsmap.find(ns.prefix ? ns.prefix : "");
    if (pos != nsmap.cend())
        pos->second++;
    else
        pos = nsmap.insert({ns.prefix ? ns.prefix : "", 1}).first;
    if(xmlTextWriterStartElementNS(w.get(),
            pcxmlChar(ns.prefix),
            pcxmlChar(name.c_str()),
            pos->second > 1 ? nullptr : pcxmlChar(ns.ns)) == -1)
        return IO_ERROR;
