...
5. **Encrypt a file with a public key from an ID card and use key server**  
   ...
   ```bash
   ./cdoc-tool encrypt --rcpt Test:p11pk:0:::Isikutuvastus --library /opt/homebrew/lib/opensc-pkcs11.so --server 00000000-0000-0000-0000-000000000000 https://cdoc2.id.ee:8443 --accept keyserver-cert.der --out abc.txt-ks.cdoc ~/Documents/abc.txt
   ```
...
3. **Decrypt a file with an ID card and use key server**  
   Decrypt the file `abc.txt-ks.cdoc` using the key label `Test` and a private key from an ID card and using the RIA key server.
